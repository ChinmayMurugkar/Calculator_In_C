...
mmenu()                          //Displaying the main menu
{
 ...
 switch(ch)
 {
  case '1': calc();		    //Displaying the calculator
	    break;       	    //Calculations

  case '2': mouse();                //Mouse interfacing
	    break;

  case '3': help();                 //Know how to use this calculator
	    break;

  case '4': about();                //Project made by
	    break;

  case '5': return('n');            //Exit from main menu

  default : sound(1000);            //Beep heard if user strikes a wrong key
	    delay(1000);
	    nosound();
...
char calc()
{
 ...
 a:while(i<eight)
 {
  ...
  switch(x)
  {
   ...
   case 'n': goto c; //for storing the number if it's less than 8 digits

   case 'q': ch='n'; //to go directly to the end
	     goto e;
  }
 }
...
void getmousepos(int *button,int *x,int *y)
{
 ...
 int86(0x33,&i,&o);
 *button=o.x.bx;    //bx=mouse button status
